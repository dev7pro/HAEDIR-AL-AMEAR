...
return 'يمكن للمطورين فقط تفعيل البوت هنا '
    end
end
    local data = load_data(_config.moderation.data)
  if data[tostring(msg.to.id)] then
if not lang then
   return 'It has been added in the protected bot'
else
 return 'تم  اضافه المجموعه ضمن حمايه البوت'
  end
end
...
local lang = redis:get(hash)
    -- superuser and admins only (because sudo are always has privilege)
     ...
     if not lang then
        ...
        return 'يمكن للمطورين فقط تفغيل البوت هنا '
    end
   end
    local data = load_data(_config.moderation.data)
  ...
  if not lang then
    ...
    return 'تم ازاله المجموعه من حمايه البوت '
   end
  end
